...
var (
	ErrResponseParserIsNotRegistered = errors.New("requestParser is not registered")
	ErrReturnDataIsNotMatched        = errors.New("number of return data is not match with number of call")
	ErrUnpackMulticallFailed         = errors.New("unpack multicall failed")
)
...
func GetResponseParser(method RequestMethod) (ResponseParser, error) {
	...
	if !ok {
		return nil, errors.Wrapf(ErrResponseParserIsNotRegistered, "method: [%s]", method)
	}
...
func ResponseParserAggregate(executor RequestExecutor, resp *Response) error {
	...
	for i, c := range resp.Request.Calls {
		...
		if err := c.ABI.UnpackIntoInterface(c.Output[0], c.Method, result.ReturnData[i]); err != nil {
			return errors.Wrap(ErrUnpackMulticallFailed, err.Error())
		}
	}
